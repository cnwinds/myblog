...
const PROTECTED_ARTICLE_ROUTES = new Set(['/articles/unpublished']);
 ...
 * @returns 是否为公开路由
 */
function isPublicRoute(url: string, method: string = 'get'): boolean {
  ...
  if (methodLower === 'get' && url.startsWith('/articles')) {
    // 排除需要认证的路由
    if (PROTECTED_ARTICLE_ROUTES.has(url)) {
      return false;
    }
    return true;
  }
